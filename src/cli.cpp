...
public:
    void run() {
        ...
        while (!exit_ && !pages_.empty()) {
            ...
            const InputEvent event = readInputEvent(animated ? 120 : 500);
...
private:
    ...
    PromptAnswer promptLine(const std::string &title,
        ...
        while (true) {
            ...
            const InputEvent event = readInputEvent(80);
            if (event.kind == InputKind::CtrlC) {
                std::raise(SIGINT);
            }
            if (event.kind == InputKind::Escape) {
    ...
    bool confirmYesNoWithBody(const std::string &title, std::vector<std::string> body, bool defaultYes = false) {
        ...
        while (true) {
            ...
            const InputEvent event = readInputEvent(1000);
            if (event.kind == InputKind::CtrlC) {
                std::raise(SIGINT);
            }
            if (isScrollInput(event)) {
...
inline int selfTest() {
    ...
    check("security 模块校验 helper", normalizePublicIpAddress("8.8.8.8", normalizedIp) &&
                                           ...
                                           configValueOr("  ", "fallback") == "fallback" &&
                                           configValueOr("  value  ", "fallback") == "value" &&
                                           policyRoleForJail(kRule1Jail).find("SSH") != std::string::npos &&
                                           policyRoleForJail("custom-web") == "自定义策略");
